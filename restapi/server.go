...
import (
	...
	"github.com/crioto/swagger-playground/restapi/operations"
)
...
// NewServer creates a new api swagger pg server but does not configure it
func NewServer(api *operations.SwaggerPgAPI) *Server {
	s := new(Server)
...
type Server struct {
	...
	SocketPath    flags.Filename `long:"socket-path" description:"the unix socket to listen on" default:"/var/run/swagger-pg.sock"`
...
func (s *Server) SetAPI(api *operations.SwaggerPgAPI) {
	if api == nil {
		s.api = nil
		s.handler = nil
		return
...
func (s *Server) Serve() (err error) {
	...
	if s.hasScheme(schemeUnix) {
		...
		wg.Add(2)
		s.Logf("Serving swagger pg at unix://%s", s.SocketPath)
		go func(l net.Listener) {
			defer wg.Done()
			if err := domainSocket.Serve(l); err != nil {
				s.Fatalf("%v", err)
			}
			s.Logf("Stopped serving swagger pg at unix://%s", s.SocketPath)
		}(s.domainSocketL)
	...
	if s.hasScheme(schemeHTTP) {
		...
		wg.Add(2)
		s.Logf("Serving swagger pg at http://%s", s.httpServerL.Addr())
		go func(l net.Listener) {
			defer wg.Done()
			if err := httpServer.Serve(l); err != nil {
				s.Fatalf("%v", err)
			}
			s.Logf("Stopped serving swagger pg at http://%s", l.Addr())
		}(s.httpServerL)
	...
	if s.hasScheme(schemeHTTPS) {
		...
		wg.Add(2)
		s.Logf("Serving swagger pg at https://%s", s.httpsServerL.Addr())
		go func(l net.Listener) {
			defer wg.Done()
			if err := httpsServer.Serve(l); err != nil {
				s.Fatalf("%v", err)
			}
			s.Logf("Stopped serving swagger pg at https://%s", l.Addr())
